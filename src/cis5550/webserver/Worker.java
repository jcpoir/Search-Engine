...
public class Worker extends Thread {
  ...
  private static Map<String, String> getQueryParams(String path) {
    ...
    logger.info("QUERY PARAM: " + separate[1]);
    ...
    for (String query : queries) {
      ...
      try {
        ...
        logger.info("KVS PAIR: " + Arrays.toString(keyValuePair));
      }
    }
  ...
  private static void sendStaticResponse(String filePath, String responseCode, Socket socket, ArrayList<String> lines)
    ...
    if (!responseCode.equals("200 OK")) {
      ...
      if (rangeRequest != null) {
        responseCode = "206 OK";
        sendHeaderResponse(socket.getOutputStream(), responseCode, getFileLengthRange(rangeRequest, filePath),
            getContentType(lines.get(0)));
      } else {
        sendHeaderResponse(socket.getOutputStream(), responseCode, getFileLength(filePath),
            getContentType(lines.get(0)));
      }

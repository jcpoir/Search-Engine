...
public class PageRankOld {
	...
	public static float DECAY = (float) 0.85; public static float CONV_THRESH = (float) 0.01; public static float CONV_P = (float) 1.0;
	...
	public static void run(FlameContext ctx, String[] args) throws FileNotFoundException, IOException {
		...
		while (crawlTable.hasNext()) {
			...
			kvs.put(prTabName, url, "url", urls); kvs.put(prTabName, url, "rank0", "1.0"); kvs.put(prTabName, url, "rank1", "1.0"); kvs.put(prTabName, url, "n", "" + urls_.size());
		}
		...
		for (int i = 0; i < 1000; i++) {
			...
			while (transfer.hasNext()) {
				...
				float val = rank1 / url_arr.length * DECAY; 
